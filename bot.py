...
ML_CONF_MIN        = 0.70  # minimum ML confidence
ML_CONF_MAX        = 0.95
MAX_ODDS           = 65    # only bet when odds below 55¢
MIN_ODDS           = 15    # only bet when odds above 15¢ (not near-resolved)
MIN_EDGE           = 0  # minimum edge (ML prob - market price)
...
def get_market_direction(up_cents, down_cents):
    """Returns the direction the market is pricing — simply which side is cheaper (more likely)"""
    if up_cents > down_cents:
        return "Up"
    elif down_cents > up_cents:
        return "Down"
    return None  # 50/50 — no clear market direction
    ...
    try:
        ...
        if _chainlink_contract is None:
            ...
            {"inputs":[],"name":"decimals","outputs":[{"name":"","type":"uint8"}],
             "stateMutability":"view","type":"function"}]
            addr = Web3.to_checksum_address("0x10C8264C0935b3B9870013e057f330Ff3e9C56dC")
...
def collect_features(ts):
    try:
        ...
        if price_to_beat is None:
            price_to_beat = float(candles[-1]["open"])
    ...
    try:
        ...
        for attempt in range(10):
            ...
            try:
                ...
                if "invalid amounts" in err:
                    print(f"  ✅ Remaining {remaining:.4f} too small — considered sold")
                    return True
                if "balance:" in err:
                ...
                err = str(e)
                if "invalid amounts" in err:
                    print(f"  ✅ Remaining too small — considered sold")
                    return True
                if "balance:" in err:
                    m = re.search(r'balance: (\d+)', err)
                    if m:
...
# ── Auto logging — saves all output to file ────────────────────────────────────
import sys
...
def setup_logging():
    ...
    log_name = f"bot_log_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    log_path = os.path.join(log_dir, log_name)
    sys.stdout = TeeLogger(log_path)
    print(f"📝 לוג נשמר ל: {log_path}")
    return log_path
...
def run_bot():
    ...
    print(f"ML conf >= {ML_CONF_MIN} | max {MAX_BETS_PER_WINDOW} bets/window")
    ...
    stats_hour_start = int(time.time())

    def print_hourly_stats():
        elapsed = int(time.time()) - stats_hour_start
        hrs     = elapsed / 3600
        print("\n" + "═"*55)
        print(f"📊 HOURLY STATS ({elapsed//60}m elapsed)")
        print(f"  חלונות סה\"כ:       {stats['windows_total']}  ({stats['windows_total']/max(hrs,0.01):.1f}/שעה)")
        print(f"  ML נעל:            {stats['ml_locked']}  ({stats['ml_locked']/max(hrs,0.01):.1f}/שעה)")
        print(f"  ML timeout:        {stats['ml_timeout']}  ← ML לא התכנס")
        print(f"  דילג PTB:          {stats['skipped_ptb']}  ← PTB חוסם")
        print(f"  דילג Market dir:   {stats['skipped_market']}  ← שוק מנוגד")
        print(f"  דילג Trend:        {stats['skipped_trend']}  ← טרנד מנוגד")
        print(f"  דילג Odds גבוה:    {stats['skipped_odds']}  ← odds > {MAX_ODDS}¢")
        print(f"  דילג Resolved:     {stats['skipped_resolved']}  ← שוק נסגר")
        print(f"  ── כניסות: {stats['entered']} | ✅ {stats['wins']} | ❌ {stats['losses']} ──")
        win_rate = stats['wins']/max(stats['entered'],1)*100
        print(f"  Win rate:          {win_rate:.0f}%")
        print(f"  רווח נטו:          ${stats['profit']:.2f}")
        print(f"  רווח/שעה:          ${stats['profit']/max(hrs,0.01):.2f}")
        # Identify biggest blocker
        blockers = {
            "ML timeout":   stats['ml_timeout'],
            "PTB":          stats['skipped_ptb'],
            "Market dir":   stats['skipped_market'],
            "Trend":        stats['skipped_trend'],
            "Odds גבוה":    stats['skipped_odds'],
        }
        top = max(blockers, key=blockers.get)
        if blockers[top] > 0:
            print(f"  🔴 חוסם ראשי:      {top} ({blockers[top]} פעמים)")
        print("═"*55 + "\n")
    ...
    while True:
        try:
                    ...
                    if pos_odds and pos["direction"] in pos_odds:
                        ...
                        if gain_pct >= TAKE_PROFIT_PCT:
                            ...
                            if success:
                                ...
                                consecutive_losses = 0
                                stats["wins"]   += 1
                                stats["profit"] += pos["amount"] * gain_pct
                            ...
                            if success:
                                ...
                                consecutive_wins = 0
                                stats["losses"] += 1
                                stats["profit"] -= pos["amount"] * STOP_LOSS_PCT
            ...
            if ts != current_ts:
                ...
                if int(time.time()) - stats_hour_start >= 3600:
                    print_hourly_stats()
                    stats_hour_start = int(time.time())
            ...
            if secs_left <= BET_SECONDS_BEFORE and ts not in placed and markets:
                ...
                # ── STEP 1: ML decides direction ──────────────────────────────
                if ts not in ml_decisions:
                    # ML timeout — if waiting too long, skip window
                    if ts in last_check and (now_epoch - last_check.get(f"ml_start_{ts}", now_epoch)) > 180:
                        print(f"[{now_str}] ⏱ ML timeout — לא התכנס תוך 3 דקות — דולג")
                        stats["ml_timeout"] += 1
                        placed.add(ts); time.sleep(1); continue

                    if f"ml_start_{ts}" not in last_check:
                        last_check[f"ml_start_{ts}"] = now_epoch
                    ...
                    if not pred:
                        print(f"[{now_str}] ML unavailable — retrying...")
                        time.sleep(1); continue
                    if pred["confidence"] < ML_CONF_MIN:
                        print(f"[{now_str}] ML {pred['confidence']:.0%} < {ML_CONF_MIN:.0%} — rechecking in 3s...")
                ...
                try:
                    sol_k = requests.get(f"{BINANCE_API}/klines",
                        ...
                        timeout=5).json()
                    ptb_price = float(sol_k[0][1]) if sol_k else None
                except: pass
                ...
                # ── PTB confirmation — STRICT: only ✓ counts, "near" is rejected ──
                ptb_confirm = False
                ptb_status  = "unknown"
                ...
                if ptb_price and cur_price:
                    ...
                    if ml_dir == "Up":
                        if beating:
                            ...
                            ptb_status  = f"✗ not beating PTB ${ptb_price:.2f} ({dist_pct:+.2f}%)"
                ...
                # ── Market direction check — replaces fair_edge ───────────────
                market_dir = get_market_direction(up_c, dn_c)
                ...
                # ── Streak sizing — REMOVED, fixed bet only ───────────────────
                streak_multiplier = 1.0

                # ── BET: ML + strict PTB agree ────────────────────────────────
                if ptb_confirm:
                    ...
                    if current_odds < MIN_ODDS:
                        print(f"  → SKIP — {ml_dir} at {current_odds}¢ < {MIN_ODDS}¢ (market nearly resolved)")
                        placed.add(ts); time.sleep(1); continue
                    ...
                    print(f"  → PTB not confirmed — waiting...")
                    stats["skipped_ptb"] += 1
